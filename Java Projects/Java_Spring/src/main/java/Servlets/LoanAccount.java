...
import configuration_classes.Loan;
...
public class LoanAccount extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		mon = 0-mon;
		ApplicationContext ctx = new AnnotationConfigApplicationContext(Loan.class);
		Loan_Account act= (Loan_Account) ctx.getBean("account");
		act.setName(name);
		act.setActNo(no);
		act.setPin(pin);
		act.setMoney(mon);
		RequestDispatcher rd = request.getRequestDispatcher("loandb");
